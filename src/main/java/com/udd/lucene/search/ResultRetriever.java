...
public class ResultRetriever {
	...
	public List<ResultDataApplication> getResultsApplication(org.elasticsearch.index.query.QueryBuilder query) {
		...
		for (var ap : applications) {
			...
			results.add(new ResultDataApplication(unit.getFirstname(), unit.getLastname(), unit.getEducation(), filename, unit.getCity(),
					/*unit.getLocation().toString()*/highlight));
		}
	...
	public List<ResultDataApplication> geoSearch(GeoSearchDTO geoSearchDTO) throws IOException {
		...
		JOpenCageForwardRequest jOpenCageForwardRequest = new JOpenCageForwardRequest(query);
		JOpenCageResponse jOpenCageResponse = jOpenCageGeocoder.forward(jOpenCageForwardRequest);
		JOpenCageLatLng locationLatLng = jOpenCageResponse.getFirstPosition();
		...
		for(SearchHit<Application> ap : searchHits) {
			...
			searchResultDTO.setCity(unit.getCity());
